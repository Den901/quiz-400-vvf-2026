@echo off
cd /d "%~dp0"
where py >nul 2>nul && (py -3 portable_server.py & exit /b)
where python >nul 2>nul && (python portable_server.py & exit /b)
echo Python 3 non e' installato ed e' necessario per Quiz 400 VVF 2026.
...
if exist "%LocalAppData%\Programs\Python\Python314\python.exe" (
  "%LocalAppData%\Programs\Python\Python314\python.exe" portable_server.py
  exit /b
)
where py >nul 2>nul && (py -3 portable_server.py & exit /b)
where python >nul 2>nul && (python portable_server.py & exit /b)
