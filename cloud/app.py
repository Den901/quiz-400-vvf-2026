    ...
    for category, count in plan.items():
        ...
        if category != "logica":
            ...
            if challenge_date >= date(2026, 9, 15) and category == 'informatica' and count:
                office = [row for row in source if str(row['id']).startswith('office-mininterno-') and row.get('image')]
                if not office:
                    raise HTTPException(503, "Quesiti del Pacchetto Office non disponibili per la Sfida del giorno.")
                selected.extend(rotating_daily_questions(office, 1, f"{seed}|pacchetto-office", usage))
                # Exactly one illustrated Office question; the remaining slots use other Informatica topics.
                source = [row for row in source if not row.get('image')]
                count -= 1
